...
For testing purposes a number of images will be taken of objects that are not part of the training set. On the contrary to the objects used for training we will not record handovers of them and instead manually decide which class they belong to. This means that we are required to choose objects that in some ways resemble the objects from the training set and that have a quite decisively way of being handed over, so it does not become too questionable about which class they belong to. The images of the objects will be inputed to the model achieved and the accuracy will be tested against it. We aim for an accuracy of 90\% with a standard deviation of 2\%. If these results are achieved we judge the model to be successful.
