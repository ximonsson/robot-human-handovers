...
The second part of the work is dedicated to analyzing image data of the objects. We will explore if RGBD images alone are enough to succesfully output the handover class of an object. If we are succesful in training a model to return the handover class of an object based on image data, then we can say that there are visual similarities between the objects belonging to the same class.


\subsubsection*{Is this model applicable to new objects?}

In case we can answer yes to the previous question we are going to want to try on a test set consisting of foreign objects. These new objects will be manually classified depending on the results from the first part, and tested on the model achieved from the second part. With high enough success rate we will consider this method as applicable for a larger set of objects where we only record and train for a small subset.
